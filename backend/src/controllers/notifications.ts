import { Router, PostgresClient } from '../deps.ts';
...
export const notificationsRouter = new Router().get('/', async (context) => {
  ...
  const newOffers = offers.filter(offer => !databaseResult.includes(offer.id)).map(offer => offer.id);
...
VALUES ${newOffers.map(offer => `(${offer})`).join(',')};`);
  ...
  const pusherUrl = `https://${Deno.env.get(
    'PUSHER_INSTANCE_ID'
  ...
  const pusherBody = {
    ...
    web: {
      notification: {
        title: 'Neue Angebote',
        body: `Es gibt ${newOffers.length === 1 ? '1 neues Angebot' : `${newOffers.length} neue Angebote`} im DHV Gebrauchtmarkt`,
        icon: 'https://www.dhv.de/dbresources/dhv/images/dhvheader2011/dhv_logo.png',
        deep_link: `https://dhv-gebrauchtmarkt-ui.netlify.app/${newOffers.length === 1 ? `offers/${newOffers[0]}` : ''}`
      },
    },
  };
