export default class SeenOffersStore {
  ...
  private getOffer(offerId: number) {
    return new Promise((resolve, reject) => {
      ...
      const request = this.dbConn.transaction('seenOfferPreviews').objectStore('seenOfferPreviews').get(offerId);
  ...
  async hasPendingNotification(offerId: number) {
    return new Promise((resolve, reject) => {
      ...
      const request = this.dbConn.transaction('pendingNotifications').objectStore('pendingNotifications').get(offerId);
