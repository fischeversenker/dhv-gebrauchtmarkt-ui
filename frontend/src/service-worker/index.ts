...
self.PusherPushNotifications = {
  ...
  _hasVisibleClient: () => self.PusherPushNotifications._getVisibleClient().then((client) => client !== undefined),
  ...
  _hasFocusedClient: () => self.PusherPushNotifications._getFocusedClient().then((client) => client !== undefined),
  ...
  reportEvent: async ({ eventType, state }) => {
    const path = `${self.PusherPushNotifications._endpoint(state.instanceId)}/reporting_api/v2/instances/${state.instanceId}/events`;
