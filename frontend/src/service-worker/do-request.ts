export default function doRequest({ method, path, body = null, headers = {}, credentials = 'same-origin' }) {
  const options = {
    method,
    headers,
    credentials
...
async function handleError(response) {
  let errorMessage;
  try {
    const { error = 'Unknown error', description = 'No description' } = await response.json();
