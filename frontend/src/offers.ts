...
export async function getOffers(offset: number) {
  isLoading.set(true);
  const receivedOffers = await fetch(`${process.env.API_BASE}/offers?search=${get(filterSearchString)}&offset=${offset}&itemsPerPage=${get(itemsPerPage)}&category=${get(filterCategory)}`).then(res => res.json());
  isLoading.set(false);
